...
class PropertyViewSet(viewsets.ModelViewSet):
    ...
    def upload_image(self, request, pk=None):
        """
        Custom Action to upload photos for a property.
        URL: /api/listings/{id}/upload_image/
        """
        property_obj = self.get_object()
        
        # 1. Check Ownership (Double check safety)
        if property_obj.owner != request.user and not request.user.is_staff:
            return Response({"error": "You do not own this property"}, status=403)
        ...
        # 3. Save it
        photo = PropertyImage.objects.create(property=property_obj, image=image_file)
        
        return Response(PropertyImageSerializer(photo).data, status=200)
