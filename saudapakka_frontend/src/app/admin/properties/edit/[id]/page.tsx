...
import {
    ...
    Phone, Video, Ruler, DollarSign, LandPlot, Sprout, Store
} from "lucide-react";
import Link from "next/link";

// --- Dynamic Imports ---
const SmartLocationPicker = dynamic(
    () => import("@/components/maps/SmartLocationPicker").catch(err => {
        // Return a fallback component if import fails
        return () => (
            <div className="h-[400px] w-full bg-gray-100 rounded-xl flex items-center justify-center">
                <p className="text-gray-500">Map component unavailable</p>
            </div>
        );
    }),
    {
...
const PROPERTY_TYPES = [
    { value: "FLAT", label: "Flat", icon: <Building2 className="w-8 h-8 mx-auto" /> },
    { value: "VILLA_BUNGALOW", label: "Villa/Bungalow", icon: <Home className="w-8 h-8 mx-auto" /> },
    { value: "PLOT", label: "Plot", icon: <LandPlot className="w-8 h-8 mx-auto" /> },
    { value: "LAND", label: "Land", icon: <Sprout className="w-8 h-8 mx-auto" /> },
    { value: "COMMERCIAL_UNIT", label: "Commercial", icon: <Store className="w-8 h-8 mx-auto" /> }
];
...
export default function EditPropertyPage() {
    ...
    const fetchProperty = async () => {
        ...
        try {
            ...
            // Image Fetching - Direct from property object to avoid 404s
            const images = property.images || [];
            // Handle thumbnail fallback if no images array
            if (images.length === 0 && property.thumbnail_image) {
                images.push({
                    id: 1,
            ...
            // Normalize floor plans to ensure consistent structure if needed, 
            // though state expects string[] or object[]. 
            // Existing state definition: useState<string[]>([]) - wait, line 190 says string[].
            // But line 407 was casting to any: { id: 1, image: property.floor_plans }.
            // The render loop (line 1402) handles both: const url = typeof fp === 'string' ? fp : fp.image;
            // So we just pass the raw array/item.
            setExistingFloorPlans(floorPlansData);
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            // NOTE: We do NOT append 'uploaded_images' here anymore as we will upload them separately below.

            // Append new floor plans
            floorPlans.forEach((file) => {
                formDataToSend.append('floor_plans', file);
            });
            ...
            await api.patch(`/api/properties/${propertyId}/`, formDataToSend, {
                headers: { "Content-Type": "multipart/form-data" },
            });

            // 2. Upload Images Separately (if any)
            if (galleryImages.length > 0) {
                const uploadPromises = galleryImages.map((file, i) => {
                    const imgData = new FormData();
                    imgData.append("image", file);
                    // Set thumbnail logic if needed (e.g. first image if no existing images)
                    ...
                    return api.post(`/api/properties/${propertyId}/upload_image/`, imgData, {
                        ...
                        // We don't throw here to allow other images to proceed, but we could track failures
                        return null;
                    });
                });
    ...
    return (
        <div className="min-h-screen bg-gray-50 flex flex-col">
            ...
            {/* Footer Actions */}
            <div className="bg-white border-t border-gray-200 p-4 fixed bottom-0 right-0 left-0 lg:left-72 z-40 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
                <div className="max-w-7xl mx-auto flex items-center justify-between">
                    <button
                        onClick={prevStep}
                        disabled={step === 1}
                        className={`px-6 py-3 rounded-xl font-bold transition border-2 border-gray-200 
                            ${step === 1 ? 'opacity-0 invisible' : 'opacity-100 visible text-gray-700 hover:bg-gray-100 hover:border-gray-300'}`}
                    >
                        Back
                    </button>
